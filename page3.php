...
$result=mysqli_query($con,$query);
	...
	else if(isset($_POST['Calculatedis']))/*cal disc*/
	{
			...
			if ($info && $row = $info->fetch_assoc()) {
				...
				// Upsert bill discount row for this order
				$con->query("INSERT INTO bill (Discount, Order_id, Table_id, Cust_id) VALUES (".(float)$disc.", " . $oid . ", '".$con->real_escape_string($tid)."', '".$con->real_escape_string($cid)."')");
	...
	else if(isset($_POST['Calculatebill']))/*cal bill*/
	{
			...
			$sumRes = $con->query("SELECT SUM(Amount) AS tot FROM Order_items WHERE Order_id=".$oid);
			$tot = 0;
			if ($sumRes && ($srow = $sumRes->fetch_assoc()) && $srow['tot'] !== null) {
				$tot = (float)$srow['tot'];
			}
			// Get discount if present
			$discRes = $con->query("SELECT Discount, Table_id, Cust_id FROM bill WHERE Order_id=".$oid." ORDER BY Bill_no DESC LIMIT 1");
	...
	else if(isset($_POST['paybill']))/*pay bill*/
	{
			...
			if ($b && ($br = $b->fetch_assoc()) && $br['Net_amount'] !== null) {
				...
				$ins->bind_param('sdi', $method, $paid, $billno);
				if ($ins->execute()) {
	...
	else if(isset($_POST['place']))/*place order*/
	{
		...
		if ($res && $row = $res->fetch_assoc()) {
			...
			$ins->bind_param('iiisssi', $price, $quantity, $amount, $fid, $foodname, $jain, $oid);
			if ($ins->execute()) {
	...
	else if(isset($_POST['varysimulate']))/*simulate a price change*/
	{
		...
		if ($pick && ($f = $pick->fetch_assoc())) {
			...
			$after = $before + 10;
			$con->query("UPDATE food SET Price=".$after." WHERE Food_id='".$con->real_escape_string($fid)."'");
			$con->query("INSERT INTO price_increase (cur_date, field_name, before_value, after_value, Food_id, Food_name) VALUES (NOW(), 'Price', " . $before . ", " . $after . ", '".$con->real_escape_string($fid)."', '".$con->real_escape_string($fname)."')");
